...
#include <string.h>
#include <stdlib.h>
int check(char rule[26]);
int strtoint(char num[]); // Doesn't error check
typedef struct node {
    char *val;
    struct node * next;
} node_t;
...
void main(){
    char list[]="15-16 m: mhmjmzrmmlmmmmmmn\n\
5-6 d: dcdddhzld\n\
3-4 s: vqssdcbl\n\
3-6 b: bbhbbbqbbb\n\
10-11 q: cqqntqhqwwh\n\
...
9-16 d: dsdddddddrdddddhdbdd";
   ...
   while (currentnode->next!=NULL){
    results=results+check(currentnode->val);
...
int check(char rule[26]){
    char * splittings = strtok(rule," ");
    char * letter = strtok(NULL, ":");
    char * password = strtok(NULL, " ");
    printf("%s,%s,%s\n",splittings, letter, password);
    char * num1 = strtok(splittings,"-");
    char * num2 = strtok(NULL, "-");
    int lower = strtoint(num1)-1;
    int upper = strtoint(num2)-1;
    int len = strlen(password);
    printf("%c\n",password[lower]);
    if(password[lower]==*letter && !(password[upper]==*letter)){
        return 1;
    }
    else if (password[upper]==*letter && !(password[lower]==*letter)){
        return 1;
    }
    else{
        return 0;
